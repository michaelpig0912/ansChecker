...
class ZoneSelector:
    def __init__(self, image_path):
        ...
        # Y方向的滑動條（用於題目高度判定）
        self.y_range = tk.IntVar(value=5)
        ...
        self.range_label_y = ttk.Label(self.range_frame_y, text="垂直高度: 5")
...
def process_page(page, csv_writer, question_intervals, score_zones):
    ...
    binary_image = cv2.adaptiveThreshold(
        ...
        9,  # 塊大小：較小的值(3-15)可以檢測到更細的線條
        2    # 常數：較小的值(1-5)會增加靈敏度
    )

    # 使用形態學操作來增強勾選標記
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))  # 核心大小：(1,1)到(3,3)
    binary_image = cv2.dilate(binary_image, kernel, iterations=1)  # 迭代次數：1-2
    binary_image = cv2.erode(binary_image, kernel, iterations=1)   # 迭代次數：1-2

    # 去除表格線
    kernel_length = max(binary_image.shape[1] // 150, binary_image.shape[0] // 150)  # 縮小kernel
    ...
    for contour in contours:
        ...
        # 更寬鬆的條件來檢測細線條的勾選標記
        if (
            8 < area < 200 and        # 面積範圍：降低最小值會增加靈敏度
            0.2 < aspect_ratio < 3.0 and  # 長寬比：放寬範圍會增加靈敏度
            5 < w < 25 and 5 < h < 25 and  # 大小範圍：降低最小值會增加靈敏度
            perimeter < 100           # 周長限制：增加值會放寬限制
        ):
            # 計算填充率
            rect_area = w * h
            fill_ratio = area / rect_area
            if 0.05 < fill_ratio < 0.5:  # 填充率：降低最小值會增加靈敏度
                filtered_contours.append(contour)

    # 在圖片上顯示檢測結果（用於調試）
    debug_image = cv2.cvtColor(binary_image_no_lines, cv2.COLOR_GRAY2BGR)
    for contour in filtered_contours:
        x, y, w, h = cv2.boundingRect(contour)
        cv2.rectangle(debug_image, (x, y), (x+w, y+h), (0, 255, 0), 1)
        cv2.circle(debug_image, (x + w//2, y + h//2), 2, (0, 0, 255), -1)

    # 保存調試圖片
    cv2.imwrite(f"debug_page_{page.number + 1}.png", debug_image)
